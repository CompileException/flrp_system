...
RegisterNetEvent("flrp_system_ampelsync:setLight")
AddEventHandler("flrp_system_ampelsync:setLight", function(coords)
    ...
    while true do
        ...
        if IsPedInAnyVehicle(player) and IsVehicleStopped(GetVehiclePedIsIn(player)) then
            ...
            if (trafficLight ~= 0) and (trafficLight ~= lastTrafficLight) then
                ...
                TriggerServerEvent('flrp_system_ampelsync:setLight', GetEntityCoords(trafficLight, false))

                if Config.Debug then
                    print(Config.Prefix .. " Ampeln gesynct!")
                end
