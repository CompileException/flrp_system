...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not Config.OnlyPolicemen or (Config.OnlyPolicemen and ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "polizei") then
			...
			if dstCheck <= 5.0 then
				...
				if dstCheck <= 0.5 then
					exports['flrp_textui']:showE("Um eine Waffe zu erhalten", "E")
...
OpenPoliceArmory = function()
	...
	local elements = {
		{ ["label"] = "Verfügbare Waffen", ["action"] = "weapon_storage" }
	}
...
OpenWeaponStorage = function()
	...
	function(data, menu)
		...
		if (DoesEntityExist(closestPed) and closestPedDst >= 5.0) or IsPedAPlayer(closestPed) then
			...
			ESX.ShowNotification("Bitte versuch es erneut.")
		...
		if IsEntityPlayingAnim(closestPed, "mp_cop_armoury", "pistol_on_counter_cop", 3) or IsEntityPlayingAnim(closestPed, "mp_cop_armoury", "rifle_on_counter_cop", 3) then
			ESX.ShowNotification("Bitte warten bis du an der Reihe bist.")
			return
		end
		...
		if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
			...
			TriggerServerEvent("flrp_system:policearmory:giveWeapon", weaponHash)
		end
...
RefreshPed = function(spawn)
	...
	ESX.TriggerServerCallback("flrp_system:policearmory:pedExists", function(Exists)
