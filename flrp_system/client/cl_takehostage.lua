...
function takeHostage()
		...
		if HasPedGotWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i]), false) then
			if GetAmmoInPedWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i])) > 0 then
                ...
                if Config.Debug then
                    print(Config.Prefix .. "Geisel genommen!")
                end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			holdingHostage = true 
			TriggerServerEvent('flrp_system_animations:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
		else
			drawNativeNotification("Niemand in der Nähe, den man als Geisel nehmen könnte!")
		end 
	end
	canTakeHostage = false 
end 

RegisterNetEvent('flrp_system_animations:syncTarget')
AddEventHandler('flrp_system_animations:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
...
RegisterNetEvent('flrp_system_animations:syncMe')
AddEventHandler('flrp_system_animations:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = GetPlayerPed(-1)
	ClearPedSecondaryTask(GetPlayerPed(-1))
	RequestAnimDict(animationLib)
	while not HasAnimDictLoaded(animationLib) do
...
RegisterNetEvent('flrp_system_animations:cl_stop')
AddEventHandler('flrp_system_animations:cl_stop', function()
	holdingHostageInProgress = false
	beingHeldHostage = false 
	...
	while true do 
		if holdingHostage then
			if IsEntityDead(GetPlayerPed(-1)) then	
				...
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent("flrp_system_animations:stop",target)
				Wait(100)
				releaseHostage()
			end 
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Drück [G] zum freilassen, [H] zum töten")
