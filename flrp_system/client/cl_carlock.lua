...
function VehicleInFront(checkDistance)
	...
	Citizen.Wait(10)
    if vehicle ~= nil and distance < (checkDistance and checkDistance or 5.0) then
		return vehicle
    else 
        return 0 
    end
...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 303)) then
		...
		cars = ESX.Game.GetVehiclesInArea(coords, 20.0)
		local carstrie = {}
		local cars_dist = {}		
		notowned = 0
		if #cars == 0 then
			exports['okokNotify']:Alert('Autoschlüssel', "Kein Fahrzeug gefunden!", 1000, 'error')
		else
			for j=1, #cars, 1 do
			...
			for i=1, #carstrie, 1 do
				...
				ESX.TriggerServerCallback('flrp_system:carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							exports['okokNotify']:Alert('Autoschlüssel', "Du hast dein Fahrzeug abgeschlossen!", 1000, 'success')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end

								TriggerServerEvent('InteractSound_SV:PlayOnSource', 'carlock', 0.5) 
						...
						elseif lock == 2 then
							...
							exports['okokNotify']:Alert('Autoschlüssel', "Dein Fahrzeug ist offen!", 1000, 'info')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
